...
const StockDividendCard: React.FC<StockDividendCardPropsProps> = ({ ticker }) => {
  const { loading, error, data } = useQuery(GET_DIVIDENDS_INFORMATION_BY_TICKER, {
            ...
            <Table>
              <TableBody>
                {([
                  ...
                  [{ en: "Dividend Yield(5Y)", kr: "평균 배당 수익(최근 5년)" }, `${summary.fiveYearAvgDividendYield} ${price.currency}`],
                  [{ en: "Dividend Rate(TTM)", kr: "연간 배당 수익(최근 1년)" }, `${summary.trailingAnnualDividendRate} ${price.currency}`],
                  [{ en: "Dividend Yield(TTM)", kr: "배당 수익률(최근 1년)" }, `${(summary.trailingAnnualDividendYield * 100).toFixed(2)}%`],
                  [{ en: "Dividend Rate(FWD)", kr: "연간 배당 수익(예상)" }, `${summary.dividendRate} ${price.currency}`],
                    ...
                    .map((history, index) => (
                      <TableRow key={history.date}>
                        ...
                        <TableCell background={`light-${(index % 2) + 1}`}>
                          <Text weight="bold">
                            {history.dividends} <Text weight="normal">{price.currency}</Text>
                          </Text>
                        </TableCell>
                      </TableRow>
                    ))}
